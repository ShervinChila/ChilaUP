...
REPLY_ERROR = """<code>بدون فاصله باید این دستور رو وارد کنی</code>"""
    ...
    if len(text)>7:
        ...
        if len(argument) == 3:
            ...
            try:
                ...
                return
        temp_msg = await message.reply("بصبر")
        try:
            messages = await get_messages(client, ids)
        except:
            await message.reply_text("قیمه ها ریخته شدن تو ماستا😐")
            return
...
async def get_users(client: Bot, message: Message):
    ...
    await msg.edit(f"{len(users)} از من استفاده کرده😎")
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        ...
        pls_wait = await message.reply("<i>ی خورده طول میکشه بصبر</i>")
        for row in query:
            chat_id = int(row[0])
            try:
